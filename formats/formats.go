...
func decode(raw map[string]any, baseDir string) (kscript.Definition, error) {
	...
	for key := range raw {
		if key != "version" && key != "vars" && key != "tasks" && key != "files" {
			return d, fmt.Errorf("unknown definition field %q", key)
		}
	}
	...
	for name, value := range tasks {
		...
		case map[string]any:
			for key := range value {
				if key != "desc" && key != "if" && key != "run" && key != "deps" {
					return d, fmt.Errorf("task %s: unknown field %q", name, key)
				}
			}
			...
			if run, ok := value["run"].(string); ok {
				...
				return d, fmt.Errorf("task %s: run must be string", name)
			}
		default:
