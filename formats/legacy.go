	...
	for name, raw := range scripts {
		...
		case []string:
			for _, command := range value {
				task.Steps = append(task.Steps, kscript.Step{Exec: &kscript.ExecSpec{Program: command}})
			}
		case []any:
			for _, item := range value {
				command, ok := item.(string)
				if !ok {
					return def, fmt.Errorf("legacy task %s: command must be string", name)
				}
				task.Steps = append(task.Steps, kscript.Step{Exec: &kscript.ExecSpec{Program: command}})
			}
		case map[string]any:
			...
			if deps, ok := value["deps"].([]any); ok {
				for _, item := range deps {
					dep, ok := item.(string)
					if !ok {
						return def, fmt.Errorf("legacy task %s: dep must be string", name)
					}
					task.Deps = append(task.Deps, dep)
				}
			}
			if command, ok := value["run"].(string); ok {
				task.Steps = []kscript.Step{{Exec: &kscript.ExecSpec{Program: command}}}
			}
		default:
